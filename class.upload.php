...
class upload {
    ...
    function init() {
        ...
        $this->typechck               = true;     // don't check the mime type against the allowed list
    ...
    function process($server_path = null) {
        ...
        if ($this->uploaded) {
            ...
            if ($this->typechck && empty($this->file_src_mime)) {
                $this->processed = false;
                $this->error = $this->translate('no_mime');
            } else if ($this->typechck && !empty($this->file_src_mime) && strpos($this->file_src_mime, '/') !== false) {
