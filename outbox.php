...
if($_SERVER['REQUEST_METHOD']=="POST"){
...

	{
		...
		mysql_query("update $Table_mailbox  set m_show='false' where Mail_id='$c[$i]'");
...
$res_box=mysql_query("select * from $Table_mailbox where UserFromID='$_SESSION[Site_User_ID]' and m_show='true' order by Mail_id desc");
...
$res_box=mysql_query("select * from $Table_mailbox where UserFromID='$_SESSION[Site_User_ID]' and m_show='true' order by Mail_id desc LIMIT $a,$b");
  ...
  <tr align="center" >
	...
	<table width="100%"  border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td width="92%" align="right" bgcolor="#187EA6" class="textwhite" dir="rtl">&nbsp;&nbsp;&nbsp;«·»—œ «·’«œ—  (&nbsp;<?=$num?>&nbsp;)</td>
...
$res_received=mysql_query("select * from $Table_usrs where user_id='$row_box[User_Id]'");

$row_received=mysql_fetch_array($res_received);

if($row_box[m_reads]<>"0"){$bg_msg="#DEECF3";}else{$bg_msg="#EEF5F9";}
/*---------------------filter words-------------------*/
   $words=mysql_fetch_array(mysql_query("select txt from $Table_pages where name='disallowwords'")); 
	  ...
	  $filter->strings=$words;  
	   ...
	   /*-------*/ 
	    $filter->text = $row_received[UserName];  
	   $UserName = $filter->filter();  
	   
	   /*--------------------------*/
?>

<tr align="center" bgcolor="<?=$bg_msg?>" onMouseMove="bgColor='#DEECF3'" onMouseOut="bgColor='<?=$bg_msg?>'">
...
<td    dir="rtl"><a href="?Plink=profile&id=<?=$row_box[User_Id]?>" class="textblack" title="<?=$UserName?>"><?=$UserName?></a></td>

<td width="47%" height="25" dir="rtl" class="textblack"><a href="?Plink=message&O_id=<?=$row_box[Mail_id]?>" class="textblack" title="<?=$MailTitle?>"><? if(strlen($MailTitle)>25){echo substr($MailTitle,0,25)."...";}else{echo $MailTitle;}?></a></td>

<td width="3%" dir="rtl"><img src="smilies/smile%20(<?=$row_box[SmilyeID]?>).gif" ></td>

</tr>

<? mysql_free_result($res_received);}?>

  <tr align="center" bgcolor="#187EA6">

    <td><input name="dell" type="hidden" id="dell" value="del_msg">

      <input name="Submit" type="image" onClick="return yesorno ('Â· «‰  „ √ﬂœ „‰ Õ–› «·—”«∆· «·„Õœœ… „‰ ’‰œÊﬁﬂ «·’«œ— ø')" value="Õ–›" src="images/del.gif" alt="Õ–›" width="20" height="17"></td>
